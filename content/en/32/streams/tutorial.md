...
For a full list of configurations of Kafka Streams please refer to this [table](/32/#streamsconfigs). 
...
Note that the `count` operator has a `Materialized` parameter that specifies that the running count should be stored in a state store named `counts-store`. This `counts-store` store can be queried in real-time, with details described in the [Developer Manual](/32/streams/developer-guide/interactive-queries.html). 
...
[Previous](/32/streams/quickstart) [Next](/32/streams/core-concepts)
