...
![](/31/images/streams-architecture-overview.jpg)
...
![](/31/images/streams-architecture-tasks.jpg)   
...
![](/31/images/streams-architecture-threads.jpg)
...
Kafka Streams provides so-called **state stores** , which can be used by stream processing applications to store and query data, which is an important capability when implementing stateful operations. The [Kafka Streams DSL](/31/streams/developer-guide/dsl-api.html), for example, automatically creates and manages such state stores when you are calling stateful operators such as `join()` or `aggregate()`, or when you are windowing a stream. 
...
![](/31/images/streams-architecture-states.jpg)   
