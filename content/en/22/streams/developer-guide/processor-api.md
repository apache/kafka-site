...
For a complete list of available API functionality, see the [Streams](/22/javadoc/org/apache/kafka/streams/package-summary.html) API docs.
...
Persistent `KeyValueStore<K, V>` | RocksDB | Yes (enabled by default) | 
  ...
  * Available [store variants](/22/javadoc/org/apache/kafka/streams/state/Stores.PersistentKeyValueFactory.html): time window key-value store, session window key-value store.
