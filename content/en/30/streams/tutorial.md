...
In this guide we will start from scratch on setting up your own project to write a stream processing application using Kafka Streams. It is highly recommended to read the [quickstart](/30/streams/quickstart) first on how to run a Streams application written in Kafka Streams if you have not done so. 
...
For a full list of configurations of Kafka Streams please refer to this [table](/30/#streamsconfigs). 
...
For detailed instructions on how to run a Streams application and observe its computing results, please read the [Play with a Streams Application](/30/streams/quickstart) section. We will not talk about this in the rest of this section. 
...
Note that the `count` operator has a `Materialized` parameter that specifies that the running count should be stored in a state store named `counts-store`. This `Counts` store can be queried in real-time, with details described in the [Developer Manual](/30/streams/developer-guide/interactive-queries.html). 
...
[Previous](/30/streams/quickstart) [Next](/30/streams/core-concepts)
